...
Reconciled repository milestone records so the existing controller-driven metasprite behavior remains documented without claiming the official `M39` Showcase milestone.
...
- `rg -n "M39|controller input|PSG Sound Effects|Metasprite" docs artifacts/reports rom/showcase -S` confirmed stale official-`M39` controller-input references were replaced with enhancement wording and that `docs/plan.md` still defines `M39 - PSG Sound Effects`.

## Exact Reconciliation Performed

- Renamed `artifacts/reports/m39_controller_input_demo.md` to `artifacts/reports/showcase_controller_input_enhancement.md`.
- Reclassified controller-input Showcase movement as an unplanned post-`M38` enhancement rather than the official `M39` milestone.
