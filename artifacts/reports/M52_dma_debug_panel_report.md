# M52 DMA Debug Panel Report

## Status
Completed

TASK_PACKET_MILESTONE: M52
NEXT_EXPECTED_MILESTONE_FROM_PLAN: M52
ALIGNMENT: PASS

## Summary

Implemented a read-only `DMA Debug` frontend panel that renders alongside the existing debug panels and refreshes every frame with the live DMA register state. Added a narrow DMA snapshot accessor on `DMA`, exposed that snapshot through `EmulatorCore`, and routed it through the existing shell-owned frontend runtime-state handoff so the UI can inspect DMA source, destination, transfer length, control, and active/idle status without modifying deterministic execution.
...
- `git status --short`
- `cmake -S . -B build`
- `git diff -- emulator/include/dma.h emulator/src/dma.cpp emulator/include/emulator_core.h emulator/src/emulator_core.cpp emulator/frontend/debug_panels/debug_panel_host.h emulator/frontend/debug_panels/debug_panel_host.cpp emulator/src/main.cpp emulator/tests/unit/test_emulator_core.cpp docs/state_snapshot.md artifacts/reports/M52_dma_debug_panel_report.md`
- `cmake --build build`
...
- Visual confirmation that the `DMA Debug` ImGui window renders cannot be automated in this environment
